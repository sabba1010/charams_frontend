...
const cards = [
  {
    ...
    description: (
      <>
        ...
        in your local area by promoting your services directly on Oppas Haven.{" "}
        <strong style={{ color: "#1a1a1a", fontWeight: 600 }}>
    ...
    description: (
      <>
        ...
        <strong style={{ color: "#fff", fontWeight: 600 }}>
          Oppas Haven provides custom listing categories for every professional pet service.
        </strong>
      </>
    ),
    width: 320,
...
const MAX_INDEX = cards.length - 3;
...
export default function ChooseListingsSection() {
  ...
  const goNext = () => {
    const next = Math.min(currentIndex + 1, MAX_INDEX);
  ...
  const handleTabClick = (cat: string) => {
    ...
    const idx = cards.findIndex(
      (c) => c.title && (c.title as string).toLowerCase().includes(cat.toLowerCase().split(" ")[0])
    );
    if (idx !== -1) setCurrentIndex(Math.min(idx, MAX_INDEX));
  };
            ...
            {cards.map((card) => {
              ...
              if (card.type === "case-study") {
                return (
                  ...
                  >
                    <h3 style={{ fontSize: 18, fontWeight: 500, color: "#222", margin: "0 0 16px", display: "flex", alignItems: "center", gap: 8 }}>
                      {card.title} <span>{card.emoji}</span>
                    </h3>
                    <div style={{ fontSize: 13.5, lineHeight: 1.65, color: "#888", margin: 0, maxWidth: "92%" }}>
                      {card.description}
                    </div>
                    <button
              ...
              if (card.type === "cta") {
                return (
                  ...
                  >
                    ...
                    <svg style={{ position: "absolute", inset: 0, width: "100%", height: "100%", opacity: 0.25 }} viewBox="0 0 320 400">
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: 12, marginTop: 36 }}>
          ...
          <button
            ...
            style={{
              width: 40, height: 40, borderRadius: "50%",
              border: "1px solid #e8e8e8", background: "#fafafa",
              cursor: currentIndex >= MAX_INDEX ? "default" : "pointer",
              display: "flex", alignItems: "center", justifyContent: "center",
              color: currentIndex >= MAX_INDEX ? "#ccc" : "#999",
              boxShadow: "0 1px 4px rgba(0,0,0,0.05)",
              opacity: currentIndex >= MAX_INDEX ? 0.5 : 1,
            }}
          >
            <Icons.ChevronRight />
          </button>
