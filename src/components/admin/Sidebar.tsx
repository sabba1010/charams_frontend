...
import {
  LayoutDashboard, FileText, Mail,
  Wallet, Search, PlusCircle, LayoutList,
  BarChart3, Megaphone, Ticket, Star,
  User, Power, ChevronDown, Briefcase, Users
} from 'lucide-react';
    ...
    const fetchCounts = async () => {
      try {
        ...
        if (data.success && Array.isArray(data.data)) {
          const pending = data.data.filter((l: any) => (l.status || 'Pending') === 'Pending').length;
          const rejected = data.data.filter((l: any) => l.status === 'Rejected').length;
          const active = data.data.filter((l: any) => l.status === 'Active').length;
          setCounts({ pending, rejected, active });
        }
      } catch (err) {
  ...
  const menuGroups: MenuGroup[] = [
    {
      ...
      items: [
        ...
        // { id: 'my-jobs', name: 'My Jobs', icon: <FileText size={18} /> },
        { id: 'all-jobs', name: 'All Jobs', icon: <Briefcase size={18} /> },
        { id: 'all-users', name: 'All Users', icon: <Users size={18} /> },
    ...
    },
    {
      ...
      items: [
        // { id: 'add-listing', name: 'Add Listing', icon: <PlusCircle size={18} /> },
        {
          ...
          hasDropdown: true,
          subItems: [
            { id: 'listings-pending', name: 'Pending', count: counts.pending, countBg: 'bg-[#f0ad4e]' },
            { id: 'listings-rejected', name: 'Rejected', count: counts.rejected, countBg: 'bg-[#d9534f]' },
            { id: 'listings-active', name: 'Active', count: counts.active, countBg: 'bg-[#5cb85c]' },
          ]
        },
    ...
    <aside className="w-64 bg-white border-r border-slate-100 flex flex-col fixed left-0 top-[72px] bottom-0 z-20">
      <div className="p-8 border-b border-slate-50">
        <Link to="/" className="flex items-center gap-3">
          <img src={logo} alt="Oppas Haven" className="h-10 w-auto" />
          <span className="text-black font-serif text-[18px] font-medium">Oppas Haven</span>
        </Link>
      </div>
